...
export default function AdminCreateProductPage() {
  ...
  const [category, setCategory] = useState<CreateProductPayload['category']>('casual')
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await createProduct({
        ...
        inventory: inv.length > 0 ? inv : undefined,
      })
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <h1 className="text-2xl font-bold text-neutral-900 mb-8">Crear producto</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <select
            id="category"
            value={category}
            onChange={(e) => setCategory(e.target.value as CreateProductPayload['category'])}
            className="w-full px-4 py-2.5 rounded-lg border border-neutral-300 bg-white text-neutral-900 focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:border-transparent"
            disabled={saving}
          >
            {CATEGORIES.map((c) => (
        ...
        <div>
          ...
          <p className="text-xs text-neutral-500 mt-1">
            Añade talla y cantidad. Si no añades ninguna, el producto quedará sin stock hasta que lo edites.
          </p>
        </div>
